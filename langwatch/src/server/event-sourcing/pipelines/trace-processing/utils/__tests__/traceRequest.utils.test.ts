...
describe("normalizeOtlpId", () => {
  it("joins OTLP/JSON Base64 IDs with W3C hexadecimal IDs", () => {
    const traceHex = "1d1d0954b4361835615ec908f9009638";
    const spanHex = "0123456789abcdef";

    expect(
      TraceRequestUtils.normalizeOtlpId(
        Buffer.from(traceHex, "hex").toString("base64"),
      ),
    ).toBe(traceHex);
    expect(
      TraceRequestUtils.normalizeOtlpId(
        Buffer.from(spanHex, "hex").toString("base64"),
      ),
    ).toBe(spanHex);
    expect(TraceRequestUtils.normalizeOtlpId(traceHex)).toBe(traceHex);
  });
});
