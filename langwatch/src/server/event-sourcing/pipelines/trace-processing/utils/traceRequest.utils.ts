...
const normalizeOtlpId = (id: string | Uint8Array): string => {
  ...
  // OTLP/JSON encodes byte fields as Base64, while several SDKs and the
  // rest of LangWatch expose W3C IDs as lowercase hex. Normalize both wire
  // representations at the ingestion boundary so spans from different
  // exporters can still join the same trace.
  if (/[+/=]/.test(id) && /^[A-Za-z0-9+/]+={0,2}$/.test(id)) {
    try {
      ...
      // Preserve non-OTLP string IDs for backwards compatibility.
    }
  }
