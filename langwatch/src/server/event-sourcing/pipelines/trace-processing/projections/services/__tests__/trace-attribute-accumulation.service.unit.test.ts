...
describe("TraceAttributeAccumulationService.extractAttributes", () => {
  describe("when OTLP carries Agent deployment identity", () => {
    it("retains version, instance, environment, and XiaoBa role identity", () => {
      const result = makeService().extractAttributes(
        makeSpan({
          resourceAttributes: {
            "service.name": "xiaoba-cli",
            "service.version": "1.4.0",
            "service.instance.id": "xiaoba-prod-2",
            "deployment.environment.name": "production",
          },
          spanAttributes: {
            "xiaoba.role.name": "researcher",
            "barena.connector.id": "xiaoba",
          },
        }),
      );
