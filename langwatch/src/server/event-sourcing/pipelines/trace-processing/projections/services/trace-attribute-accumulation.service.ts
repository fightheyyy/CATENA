...
export const RESOURCE_ATTR_MAPPINGS = [
  ...
  // Older SDKs emitted the pre-stable key. Retain it separately so the
  // registry can prefer the stable key while still identifying deployments
  // from already-instrumented runtimes.
  ["deployment.environment", "deployment.environment"],
] as const;

export const SPAN_ATTR_MAPPINGS = [
  // XiaoBaOS names the active product role explicitly. Keep the source key
  // honest, then let the Spiral Agent Registry use it as the Agent identity
  // fallback when the runtime does not emit the standard gen_ai.agent fields.
  ["xiaoba.role.name", "xiaoba.role.name"],
  ["barena.connector.id", "barena.connector.id"],
