...
export class TraceListService {
  ...
  /**
   * Return the OTLP-discovered Agent/deployment read model. This deliberately
   * bypasses the trace-list evaluation join: the registry is an aggregate over
   * retained telemetry, not a second Agent database.
   */
  async getAgentRegistry(params: {
    tenantId: string;
    timeRange: { from: number; to: number; live?: boolean };
  }): Promise<AgentRegistryRow[]> {
    return this.repository.findAgentRegistry(params);
  }
