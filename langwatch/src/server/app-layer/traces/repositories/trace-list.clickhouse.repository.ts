...
import { TRACE_ORIGIN_CLICKHOUSE_EXPRESSION } from "../derive-trace-origin";
...
export class TraceListClickHouseRepository implements TraceListRepository {
  ...
  async findAgentRegistry(params: {
    tenantId: string;
    timeRange: { from: number; to: number; live?: boolean };
  }): Promise<AgentRegistryRow[]> {
    EventUtils.validateTenantId(
      { tenantId: params.tenantId },
      "TraceListClickHouseRepository.findAgentRegistry",
    );
    ...
    const agentNameExpression =
      "if(Attributes['gen_ai.agent.name'] != '', Attributes['gen_ai.agent.name'], Attributes['xiaoba.role.name'])";
    const environmentExpression =
      "if(Attributes['deployment.environment.name'] != '', Attributes['deployment.environment.name'], Attributes['deployment.environment'])";
    const result = await client.query({
      query: `
        SELECT
          ...
          argMax(Attributes['barena.connector.id'], OccurredAt) AS connector_id,
          argMax(Attributes['service.version'], OccurredAt) AS service_version,
          Attributes['service.instance.id'] AS service_instance_id,
          ${environmentExpression} AS deployment_environment,
          count() AS trace_count,
          countIf(${TRACE_STATUS_CLICKHOUSE_EXPRESSION} = 'error') AS error_count,
          countIf(${TRACE_STATUS_CLICKHOUSE_EXPRESSION} = 'warning') AS warning_count,
          max(toUnixTimestamp64Milli(OccurredAt)) AS last_seen_at,
          argMax(${TRACE_STATUS_CLICKHOUSE_EXPRESSION}, OccurredAt) AS latest_status,
          arraySlice(arraySort(arrayDistinct(arrayFlatten(groupArray(32)(Models)))), 1, 8) AS models,
          groupUniqArray(8)(${TRACE_ORIGIN_CLICKHOUSE_EXPRESSION}) AS origins
        FROM ${TABLE_NAME}
        WHERE ${whereClause}
    ...
    const rows = await result.json<{
      ...
      latest_status: "ok" | "error" | "warning";
      models: string[];
      origins: string[];
    }>();
    return rows.map((row) => ({
      ...
      serviceVersion: row.service_version || null,
      serviceInstanceId: row.service_instance_id || null,
      deploymentEnvironment: row.deployment_environment || null,
