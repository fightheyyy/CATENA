...
/** Evaluator execution selected by the server before the isolated child starts. */
export const ScenarioEvaluatorRuntimeSchema = z.discriminatedUnion("type", [
  z.object({ type: z.literal("project-model") }),
  z.object({
    type: z.literal("xiaoba"),
    endpoint: z.string().url(),
  }),
]);
export type ScenarioEvaluatorRuntime = z.infer<
  typeof ScenarioEvaluatorRuntimeSchema
>;
...
export const ChildProcessJobDataSchema = z.object({
  ...
  /** Absent on older queued jobs, which retain project-model behavior. */
  evaluatorRuntime: ScenarioEvaluatorRuntimeSchema.optional(),
  nlpServiceUrl: z.string(),
