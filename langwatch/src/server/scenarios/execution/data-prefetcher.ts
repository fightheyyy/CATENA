...
export interface DataPrefetcherDependencies {
  ...
  /** Optional platform-owned evaluator. Undefined keeps upstream behavior. */
  evaluatorRuntime?: ScenarioEvaluatorRuntime;
}
...
export async function prefetchScenarioData(
  ...
  // Spiral reuses Scenario's complete HTTP Explore surface but owns the two
  // evaluator agents. The HTTP target itself needs no model, so this path can
  // run without asking each project to configure a second provider merely for
  // User Simulator/Judge execution. Other target types retain their upstream
  // model requirements.
  if (
    adapterData.type === "http" &&
    deps.evaluatorRuntime?.type === "xiaoba"
  ) {
    logger.debug(
      {
        projectId: context.projectId,
        scenarioId: context.scenarioId,
        targetType: target.type,
        evaluatorRuntime: "xiaoba",
      },
      "Prefetch complete with embedded XiaoBaOS evaluator",
    );
    return {
      success: true,
      data: {
        ...
        scenario,
        adapterData,
        evaluatorRuntime: deps.evaluatorRuntime,
  ...
  return {
    ...
    data: {
      ...
      evaluatorRuntime: { type: "project-model" },
      nlpServiceUrl: env.LANGWATCH_NLP_SERVICE,
      target,
    },
    telemetry: {
...
export function createDataPrefetcherDependencies(): DataPrefetcherDependencies {
  ...
  return {
    ...(env.SPIRAL_RUNNER_URL && {
      evaluatorRuntime: {
        type: "xiaoba" as const,
        endpoint: env.SPIRAL_RUNNER_URL,
      },
    }),
    scenarioFetcher: {
