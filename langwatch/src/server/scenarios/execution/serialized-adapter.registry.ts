...
export const SERIALIZED_ADAPTER_FACTORIES: Record<string, AdapterFactory> = {
  prompt: ({ data, modelParams, nlpServiceUrl }) =>
    new SerializedPromptConfigAdapter(
      ...
      requireModelParams(modelParams, "prompt"),
      nlpServiceUrl,
    ),
...
function requireModelParams(
  value: LiteLLMParams | undefined,
  targetType: string,
): LiteLLMParams {
  if (!value) {
    throw new Error(`Model configuration is required for ${targetType} targets`);
  }
  return value;
}
