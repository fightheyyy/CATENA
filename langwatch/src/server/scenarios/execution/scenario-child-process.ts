...
async function executeScenario(jobData: ChildProcessJobData): Promise<void> {
  ...
  let userSimulator: ScenarioRunner.AgentAdapter;
  let judgeAgent: ScenarioRunner.AgentAdapter;
  let traceAwareJudge:
    | RemoteSpanJudgeAgent
    | XiaobaScenarioReviewer
    | undefined;

  if (evaluatorRuntime?.type === "xiaoba") {
    if (target.type !== "http") {
      throw new Error(
        "Embedded XiaoBaOS Scenario evaluation currently requires a registered HTTP Agent target",
      );
    }
    const evaluatorClient = new XiaobaScenarioEvaluatorClient({
      endpoint: evaluatorRuntime.endpoint,
      ...
      runId:
        ...
        context.batchRunId,
      telemetryEndpoint: langwatchEndpoint,
      apiKey: langwatchApiKey,
    });
    userSimulator = new XiaobaScenarioUserSimulator(evaluatorClient);
    traceAwareJudge = new XiaobaScenarioReviewer({
      client: evaluatorClient,
      criteria: scenario.criteria,
      projectId: context.projectId,
      querySpans,
    });
    judgeAgent = traceAwareJudge;
  } else {
    if (!modelParams) {
      throw new Error(
        "Scenario project-model evaluation requires model configuration",
      );
    }
    // Older jobs only carried modelParams, so fall back to it when the split
    // params are absent — preserves upstream behavior outside Spiral.
    ...
    userSimulator = ScenarioRunner.userSimulatorAgent({ model: simulatorModel });
    if (target.type === "http") {
      traceAwareJudge = new RemoteSpanJudgeAgent({
        criteria: scenario.criteria,
        model: judgeModel,
        projectId: context.projectId,
        querySpans,
  ...
  if (traceAwareJudge && adapter instanceof SerializedHttpAgentAdapter) {
    bridgeTraceIdFromAdapterToJudge({ adapter, judge: traceAwareJudge });
  }
