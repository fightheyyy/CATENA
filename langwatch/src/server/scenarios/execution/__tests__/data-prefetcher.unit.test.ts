  ...
  describe("model selection", () => {
    ...
    describe("given an HTTP agent target", () => {
      ...
      describe("when prefetching scenario data", () => {
        ...
        it("uses embedded XiaoBaOS evaluators without project model configuration in Spiral mode", async () => {
          const modelResolver = {
            resolve: vi.fn().mockRejectedValue(new Error("no model configured")),
          };
          const modelParamsProvider: ModelParamsProvider = {
            prepare: vi.fn().mockRejectedValue(new Error("must not be called")),
          };
          const deps = createMockDeps({
            agentFetcher: {
              findById: vi.fn().mockResolvedValue(httpAgent),
          ...
          const result = await prefetchScenarioData(
            defaultContext,
            { type: "http", referenceId: "agent_123" },
            deps,
          );

          expect(result.success).toBe(true);
          expect(modelResolver.resolve).not.toHaveBeenCalled();
          expect(modelParamsProvider.prepare).not.toHaveBeenCalled();
          if (result.success) {
            expect(result.data.evaluatorRuntime).toEqual({
              type: "xiaoba",
              endpoint: "http://spiral-runner:8790",
            });
