...
/** Fail fast instead of serving an OAuth-only sign-in loop with no provider. */
export const assertSelectedSocialProviderConfigured = (
  e: Pick<
    typeof env,
    "NEXTAUTH_PROVIDER" | "GITHUB_CLIENT_ID" | "GITHUB_CLIENT_SECRET"
  >,
): void => {
  if (e.NEXTAUTH_PROVIDER !== "github") return;
  const missing = [
  ...
  if (missing.length > 0) {
    throw new Error(
      `NEXTAUTH_PROVIDER=github requires ${missing.join(" and ")}`,
    );
  }
};
...
if (!isBuildTime) assertSelectedSocialProviderConfigured(env);
