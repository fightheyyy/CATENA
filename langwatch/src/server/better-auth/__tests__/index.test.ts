...
describe("better-auth config", () => {
  ...
  describe("when NEXTAUTH_PROVIDER selects github", () => {
    it("fails fast when either OAuth credential is missing", async () => {
      const { assertSelectedSocialProviderConfigured } = await import(
        "../index"
      );
      expect(() =>
        assertSelectedSocialProviderConfigured({
    ...
    it("accepts a complete GitHub OAuth configuration", async () => {
      const { assertSelectedSocialProviderConfigured } = await import(
        "../index"
      );
      expect(() =>
