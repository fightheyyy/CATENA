...
export const ModelProviderSetup: React.FC<ModelProviderSetupProps> = ({
  ...
  const [state, actions] = useModelProviderForm({
    ...
    isUsingEnvVars,
    requiredDefaultChatRoles:
      variant === "onboarding" ? ["DEFAULT", "FAST", "LANGY"] : undefined,
    onSuccess: () => {
      if (onComplete) {
        onComplete();
        return;
  ...
  const [defaultModelError, setDefaultModelError] = useState<string>();
  ...
  useEffect(() => {
    ...
    setFieldErrors({});
    clearApiKeyError();
    setDefaultModelError(void 0);
  }, [modelProviderKey, clearApiKeyError]);

  const customChatModelOptions = useMemo(
    () =>
      (state.customModels ?? []).map((model) => ({
        ...model,
        value: `${backendModelProviderKey}/${model.modelId}`,
      })),
    [backendModelProviderKey, state.customModels],
  );

  // Adding one explicit custom model is already an unambiguous choice. Select
  // it in state instead of relying on the browser's visual first-option
  // fallback, which previously displayed a model while persisting null.
  useEffect(() => {
    if (
      variant === "onboarding" &&
      !state.projectDefaultModel &&
      customChatModelOptions.length === 1
    ) {
      actions.setProjectDefaultModel(customChatModelOptions[0]!.value);
    }
  }, [actions, customChatModelOptions, state.projectDefaultModel, variant]);
  ...
  const handleSaveAndContinue = useCallback(async () => {
    ...
    if (variant === "onboarding" && !state.projectDefaultModel) {
      setDefaultModelError(
        "Choose a default chat model before continuing. The Barena evaluation engine uses it for user simulation, inspection, and review.",
      );
      return;
    }
    setDefaultModelError(void 0);
      ...
      <VStack align="stretch" gap={4}>
        <HStack gap={6}>
          {backendModelProviderKey === "azure" && (
            <Field.Root invalid={Boolean(defaultModelError)}>
              <Switch
        ...
        <>
          ...
          {PROVIDERS_WITH_WELL_KNOWN_MODELS.has(backendModelProviderKey) ? (
            <Field.Root>
              ...
              {defaultModelError && (
                <Field.ErrorText>{defaultModelError}</Field.ErrorText>
              )}
            </Field.Root>
          ) : (
            <VStack align="stretch" gap={4}>
              ...
              <Field.Root invalid={Boolean(defaultModelError)}>
                ...
                <NativeSelect.Root size="sm" bg="bg.muted/40">
                  ...
                  >
                    <option value="">Select default model...</option>
                    {customChatModelOptions.map((model) => (
                      <option key={model.modelId} value={model.value}>
                        {model.displayName}
                      </option>
                    ))}
                  </NativeSelect.Field>
