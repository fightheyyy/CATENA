...
const isBarenaRecentPath = (path: string) =>
  path === "/settings" ||
  /^\/[^/]+(?:$|\/(?:agent-registry|traces|simulations|evolution)(?:\/|$))/.test(
    path,
  );
...
export function useCommandBarItems(
  ...
  const availableTopLevelNav = isBarenaMode
    ? barenaNavigationCommands
    : topLevelNavigationCommands;
  ...
  const recentItemsLimited = useMemo(() => {
    ...
    return allRecent
      .filter((item) => !isBarenaMode || isBarenaRecentPath(item.path))
      .slice(0, RECENT_ITEMS_DISPLAY_LIMIT);
  }, [groupedItems, isBarenaMode]);
  ...
  const allItems = useMemo<ListItem[]>(() => {
    ...
    if (query === "") {
      ...
      for (const result of searchResults.filter(
        (item) => !isBarenaMode || item.type === "trace",
      )) {
