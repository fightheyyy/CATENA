...
const BARENA_ACTION_IDS = new Set([
  "action-new-scenario",
  "action-send-trace",
...
export function useFilteredCommands(
  ...
  const availableNavCommands = useMemo(() => {
    const productCommands = isBarenaMode
      ? barenaNavigationCommands
      : navigationCommands;
    const commands = hasOpsAccess
      ? productCommands
      : productCommands.filter((cmd) => !cmd.id.startsWith("nav-ops"));
    return commands;
  }, [hasOpsAccess, isBarenaMode]);
  ...
  }, [isBarenaMode, isDevMode, hasOpsAccess, featureFlagOverrides]);
  ...
  const opsPinCommand = useMemo<Command[]>(() => {
    if (isBarenaMode || !hasOpsAccess) return [];
  ...
  }, [hasOpsAccess, isBarenaMode, featureFlagOverrides]);
  ...
  const availableActionCommands = useMemo(() => {
    ...
    if (isBarenaMode) {
      commands = commands.filter((cmd) => BARENA_ACTION_IDS.has(cmd.id));
    }
