...
export const navigationCommands: Command[] = [
  ...
  },
  {
    ...
    description: "Explore traces and spans",
    icon: Bird,
    category: "navigation",
    keywords: [
      "traces",
  ...
  },
  {
    id: "nav-evolution",
    label: "Evolution",
    description: "Turn trace evidence into replay and release decisions",
    icon: Activity,
    category: "navigation",
    keywords: ["evolution", "release", "replay", "case", "barena"],
  ...
  },
  {
    ...
    label: "API Keys",
    description: "Connect Agents and OTLP exporters",
    icon: Key,
    category: "navigation",
    keywords: ["api", "key", "credential", "otlp", "connect", "agent"],
...
/** Product destinations intentionally exposed by the Spiral shell. */
export const barenaNavigationCommands: Command[] = barenaNavigationIds
  .map((id) => navigationCommands.find((command) => command.id === id)!)
  .map((command) => {
    if (command.id === "nav-agents") {
      return {
        ...command,
        label: "Agents",
        description: "OTLP-discovered Agent and runtime deployments",
        path: "/[project]/agent-registry",
      };
    }
    if (command.id === "nav-traces-v2") {
      return {
        ...command,
        label: "Trace",
        description: "Observe Agent traces",
      };
    }
    if (command.id === "nav-scenarios") {
      return {
        ...command,
        label: "Explore",
        description: "Explore Agent behavior with simulated users",
      };
    }
    return command;
  });
