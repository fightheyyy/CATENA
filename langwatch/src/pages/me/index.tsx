...
function BarenaPersonalHomeRedirect() {
  ...
  if (personalProject) {
    return <Navigate to={`/${personalProject.slug}`} replace />;
  }
...
function MeEntryPage() {
  const publicEnv = usePublicEnv();
  if (!publicEnv.data) return null;
  return publicEnv.data.IS_BARENA_MODE ? (
    <BarenaPersonalHomeRedirect />
  ) : (
    <MyUsagePage />
  );
}
