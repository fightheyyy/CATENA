...
export function createEnvConfig() {
  ...
  _env = createEnv({
    ...
    server: {
      ...
      // Internal Barena evolution control plane. Never expose this value
      // through publicEnv; browser traffic enters through the tRPC gateway.
      BARENA_CONTROL_PLANE_URL: z.string().url().optional(),
      // Internal execution-plane endpoint. When set, registered HTTP Scenario
      // runs use the embedded XiaoBaOS UserCat/ReviewerCat evaluators.
      SPIRAL_RUNNER_URL: z.string().url().optional(),
      // Shared only by the LangWatch server and Barena control plane. It signs
      // project context so browser callers cannot forge cross-project access.
      BARENA_GATEWAY_SECRET: z.string().min(32).optional(),
    ...
    runtimeEnv: {
      ...
      BARENA_CONTROL_PLANE_URL: process.env.BARENA_CONTROL_PLANE_URL,
      SPIRAL_RUNNER_URL: process.env.SPIRAL_RUNNER_URL,
      BARENA_GATEWAY_SECRET: process.env.BARENA_GATEWAY_SECRET,
