...
export function ScenarioFormDrawer(props: ScenarioFormDrawerProps) {
  ...
  const executeSaveAndRun = useCallback(
    async (target: NonNullable<TargetValue>, models?: ModelOverrides) => {
      ...
      try {
        await form.handleSubmit(async (data) => {
          ...
          // would race with our redirect (lw#3586 F11).
          ...
          // The simulations page receives progress over SSE. In Spiral mode
          // the child process selects the embedded XiaoBaOS evaluator, so no
          // project simulator/judge model override is needed here.
  ...
  const handleSaveAndRun = useCallback(
    async (target: TargetValue) => {
      ...
      if (isBarenaMode) {
        await executeSaveAndRun(target);
        return;
      }
