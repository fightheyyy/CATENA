...
export function ScenarioAIGeneration({ form }: ScenarioAIGenerationProps) {
  ...
  const isDefaultModelDisabled = !defaultModelState.ok && !isBarenaMode;
  ...
  if (viewMode === "prompt") {
    // Spiral's Scenario runner gets its UserCat and ReviewerCat from the
    // embedded XiaoBaOS evaluator. A project model provider is optional, so
    // do not show LangWatch's original provider warning in the editor.
    if (isBarenaMode && !defaultModelState.ok) return null;
