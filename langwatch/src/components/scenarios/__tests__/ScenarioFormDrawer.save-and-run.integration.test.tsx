...
const runtimeMode = vi.hoisted(() => ({ isBarenaMode: false }));

vi.mock("~/hooks/usePublicEnv", () => ({
  usePublicEnv: () => ({
    data: { IS_BARENA_MODE: runtimeMode.isBarenaMode },
  }),
}));
...
describe("<ScenarioFormDrawer /> save-and-run data-loss regression", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    runtimeMode.isBarenaMode = false;
  ...
  describe("given the drawer is in edit mode with an existing scenario", () => {
    describe("when save succeeds and run is dispatched (fire-and-forget)", () => {
      ...
      it("runs immediately with the embedded evaluator in Spiral mode", async () => {
        runtimeMode.isBarenaMode = true;
        const user = userEvent.setup();
        renderEditModeDrawer();
        ...
        expect(screen.queryByTestId("confirm-run-button")).not.toBeInTheDocument();
        expect(mocks.mockRunScenario).toHaveBeenCalledTimes(1);
        expect(mocks.mockRouterPush).toHaveBeenCalledWith(
          expect.stringMatching(/^\/my-project\/simulations\?pendingBatch=/),
        );
      });
