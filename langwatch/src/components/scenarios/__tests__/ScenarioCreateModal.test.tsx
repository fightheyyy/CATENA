...
let mockIsBarenaMode = false;
vi.mock("~/hooks/usePublicEnv", () => ({
  usePublicEnv: () => ({ data: { IS_BARENA_MODE: mockIsBarenaMode } }),
}));

afterEach(() => {
  mockIsBarenaMode = false;
});
...
describe("<ScenarioCreateModal/>", () => {
  ...
  describe("when no model providers are configured", () => {
    ...
    it("uses the embedded Barena scenario draft path instead of blocking creation", () => {
      mockIsBarenaMode = true;
      render(<ScenarioCreateModal open={true} onClose={vi.fn()} />, {
        wrapper: Wrapper,
      });
      ...
      const dialog = getDialogContent();
      expect(
        within(dialog).queryByText("Model provider not ready"),
      ).not.toBeInTheDocument();
      expect(within(dialog).getByRole("textbox")).toBeInTheDocument();
      expect(
        within(dialog).getByRole("button", {
          name: "Generate with AI using Barena",
        }),
      ).toBeInTheDocument();
      expect(within(dialog).getByText("Review scenario")).toBeInTheDocument();
    });
  });
});
