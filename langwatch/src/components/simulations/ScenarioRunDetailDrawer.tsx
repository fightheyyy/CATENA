...
export function ScenarioRunDetailDrawer({
  ...
  const canAdoptEvidence =
    !!scenarioRunId &&
    scenarioState?.metadata?.langwatch?.targetType === "http" &&
    isTerminalStatus(String(scenarioState.status));
  const adoptEvidence = api.barena.adoptScenarioRun.useMutation({
    onSuccess: (result) => {
      toaster.create({
        title: t("Explore evidence retained"),
        description: t(
      ...
      void router.push(
        `/${project.slug}/evolution?newIssue=1&runId=${encodeURIComponent(result.run.run_id)}&traceId=${encodeURIComponent(result.primary_trace_id)}`,
      );
    },
    onError: (error) => {
      toaster.create({
        title: t("Evidence could not be retained"),
        description: error.message,
        type: "error",
      });
    },
  });
  const handleAdoptEvidence = useCallback(() => {
    if (!project?.id || !scenarioRunId) return;
    adoptEvidence.mutate({ projectId: project.id, scenarioRunId });
  }, [adoptEvidence, project?.id, scenarioRunId]);
                ...
                >
                  ...
                  <HStack gap={1} flexShrink={0}>
                    <ScenarioRunActions
                      ...
                      onAdoptEvidence={
                        canAdoptEvidence ? handleAdoptEvidence : null
                      }
                      isAdoptingEvidence={adoptEvidence.isPending}
                      adoptEvidenceLabel={t("Retain in Evolution")}
                    />
                    <Drawer.CloseTrigger />
                  </HStack>
                </HStack>
...
function isTerminalStatus(value: string): boolean {
  return ["SUCCESS", "FAILED", "ERROR", "CANCELLED", "STALLED"].includes(value);
}
