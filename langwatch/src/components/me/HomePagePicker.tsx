...
export function HomePagePicker({ organizationId }: Props) {
  ...
  const setMutation = api.user.setLastHomePath.useMutation({
    ...
    onError: (error) =>
      showErrorToast({
        error,
        fallbackTitle: t("saveHomePreferenceError"),
      }),
  });
  ...
  const options = useMemo<PickerOption[]>(() => {
    ...
    const opts: PickerOption[] = [
      ...
      },
      {
        ...
        description: t("personalHomeDescription"),
        path: "/me",
      },
    ];
    if (firstProjectSlug) {
      opts.push({
        value: "project",
        label: t("projectHome"),
        description: t("projectHomeDescription", {
    ...
    if (persona === "governance_admin") {
      opts.push({
        value: "governance",
        label: t("governanceOverview"),
        description: t("governanceOverviewDescription"),
        path: "/settings/governance",
      });
    }
    return opts;
  }, [stateQuery.data?.firstProjectSlug, resolverQuery.data?.persona, t]);
