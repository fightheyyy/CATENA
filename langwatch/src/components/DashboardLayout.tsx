...
const Breadcrumbs = ({ currentRoute }: { currentRoute: Route | undefined }) => {
  ...
  return (
    <HStack gap={2} fontSize="13px" color="fg.muted" alignItems="center">
      ...
      <Link href={`/${project?.slug ?? ""}`}>{t("Dashboard")}</Link>
      {currentRoute.parent && (
        <>
          <ChevronRight width="12" style={{ minWidth: "12px" }} />
          <Link
...
export type DashboardLayoutProps = {
  ...
  orgScope?: boolean;
  /**
   * Override the default `{platform} - {project.name}` tab title.
   ...
   * "Barena - Personal Workspace" because the user has no active
...
export const DashboardLayout = ({
  ...
  const platformName = isBarenaMode ? "Catena" : "LangWatch";
    ...
    >
      ...
      >
        ...
        <HStack gap={compactMenu ? 3 : 0} flex={1} alignItems="center">
          ...
          ) : isBarenaMode ? null : isOrgScopeRoute && organization ? (
        ...
        >
          ...
          >
            <VStack width="full" gap={0} {...props}>
              {/* Alert banners */}
              {publicEnv.data &&
                routeNeedsEvaluationServices(router.pathname) &&
...
export function routeNeedsEvaluationServices(pathname: string): boolean {
  return /\/(?:online-evaluations|evaluations|evaluators|workflows)(?:\/|$)/.test(
    pathname,
  );
}
