...
export type FormSnapshot = {
  ...
  /**
   * Onboarding can require one explicit chat model to back every interactive
   * role. When present, these assignments are part of the onboarding
   * transaction: navigation must not continue if any write fails.
   */
  requiredDefaultChatRoles?: Array<"DEFAULT" | "FAST" | "LANGY">;
  /**
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      if (useAsDefaultProvider) {
        ...
        const scheduleRole = (
          scope: (typeof targetScopes)[number],
          role: "DEFAULT" | "FAST" | "LANGY" | "EMBEDDINGS",
          model: string | null,
          label: string,
        ) => {
          if (!model) return;
          const key = `${scope.scopeType}:${scope.scopeId}:${role}`;
        ...
        for (const s of targetScopes) {
          const defaultRoles = requiredDefaultChatRoles ?? ["DEFAULT"];
          for (const role of defaultRoles) {
            scheduleRole(
              s,
              role,
              projectDefaultModel,
              `${role} at ${s.scopeType.toLowerCase()}`,
            );
          }
          scheduleRole(
            s,
            "FAST",
            projectTopicClusteringModel,
            `Fast at ${s.scopeType.toLowerCase()}`,
          );
          scheduleRole(
            s,
            "EMBEDDINGS",
            projectEmbeddingsModel,
            `Embeddings at ${s.scopeType.toLowerCase()}`,
          );
        }
        if (requiredDefaultChatRoles && writes.length === 0) {
          throw new Error(
            "Onboarding could not resolve a writable scope for the default model",
          );
        }
        ...
        if (requiredDefaultChatRoles && failed.length > 0) {
          throw new Error(
            `Onboarding default model assignments failed: ${failed
