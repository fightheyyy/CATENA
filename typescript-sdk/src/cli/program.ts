...
export function buildProgram({ bin }: { bin?: string } = {}): Command {
  ...
  ingestCmd
    .command("backfill <tool>")
    .description(
      "Import persisted local Agent history into the tool's configured OTLP project (codex supported today).",
    )
    .option("--dry-run", "scan and count history without uploading it")
    .option("--no-archived", "exclude ~/.codex/archived_sessions")
    .option(
      "--days <days>",
      "import the most recent 1-30 days (the platform rejects older spans)",
      (value) => Number.parseInt(value, 10),
      30,
    )
    .option(
      "--confirm-sensitive-history",
      "confirm that prompts, replies, and tool output may be uploaded",
    )
    .option("--json", "emit machine-readable JSON")
    .action(
      async (
        ...
        try {
          const { backfillCommand } = await import(
            "./commands/ingestion/backfill.js"
          );
          await backfillCommand(tool, {
            ...options,
