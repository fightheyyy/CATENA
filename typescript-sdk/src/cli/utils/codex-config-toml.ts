...
export interface CodexTraceExporterTarget {
	endpoint: string;
	token: string | null;
}

/**
 * Read the trace target already used by plain Codex Desktop. Historical
 * backfill deliberately reuses this target so a local transcript can never be
 * sent to a different project by an unrelated CLI login.
 *
 * Both Codex-supported header shapes are accepted:
 *
 * - `headers = { "Authorization" = "Bearer ..." }`
 * - `[otel.trace_exporter."otlp-http".headers]` followed by `Authorization = ...`
 */
export function readCodexTraceExporterTarget(
	filePath: string = defaultCodexConfigPath(),
): CodexTraceExporterTarget | null {
	let content: string;
	try {
	...
	const baseHeader = '[otel.trace_exporter."otlp-http"]';
	const nestedHeader = '[otel.trace_exporter."otlp-http".headers]';
	const start = content.indexOf(baseHeader);
	...
	for (const [index, line] of lines.entries()) {
		...
		if (
			index > 0 &&
			trimmed.startsWith("[") &&
			trimmed !== nestedHeader
		) {
			break;
		}
		block.push(line);
	}
	const traceBlock = block.join("\n");
	const endpoint = /^\s*endpoint\s*=\s*"([^"]+)"/m.exec(traceBlock)?.[1];
	if (!endpoint) return null;
	const token =
		/["']?Authorization["']?\s*=\s*"Bearer\s+([^"]+)"/i.exec(
			traceBlock,
		)?.[1] ?? null;
