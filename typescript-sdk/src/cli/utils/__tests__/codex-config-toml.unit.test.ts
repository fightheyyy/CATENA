...
describe("readCodexTraceExporterTarget", () => {
  it("reads the nested Authorization header used by Codex Desktop", () => {
    const filePath = path.join(tmp, "config.toml");
    fs.writeFileSync(
      filePath,
      [
        `[otel.trace_exporter."otlp-http"]`,
        `endpoint = "http://localhost:5670/api/otel/v1/traces"`,
        `protocol = "binary"`,
        ``,
        `[otel.trace_exporter."otlp-http".headers]`,
        `Authorization = "Bearer trace-write-key"`,
        ``,
        `[projects."/tmp"]`,
        `trust_level = "trusted"`,
      ].join("\n"),
    ...
    expect(readCodexTraceExporterTarget(filePath)).toEqual({
      endpoint: "http://localhost:5670/api/otel/v1/traces",
      token: "trace-write-key",
    });
  });

  it("reads an inline Authorization header without crossing into another section", () => {
    const filePath = path.join(tmp, "config.toml");
    fs.writeFileSync(
      filePath,
      [
